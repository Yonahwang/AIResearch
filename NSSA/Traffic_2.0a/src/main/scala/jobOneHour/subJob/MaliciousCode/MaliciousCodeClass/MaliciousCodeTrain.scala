...
/**
  * Created by TTyb on 2017/9/28.
  */

case class URID(label: Double, urltext: String) extends Serializable {}

class MaliciousCodeTrain(properties: Properties, spark: SparkSession, httpData: DataFrame) extends LoggerSupport with Serializable with ConfusionMatrix {
  //保存训练集文件
  def saveTrainFile(): Unit = {
    ...
    //获得负样本
    val negativeData = divideurl(getField(getData(negativeDataPath)), 0.00)
  ...
  //对uri进行分词
  def divideurl(uriData: DataFrame, label: Double): DataFrame = {
    val arr = List("%20", "%21", "%22", "%23", "%24", "%26", "%27", "%28", "%29", "%2A", "%2B", "%2C", "%3A", "%3B", "%3C", "%3D", "%3E", "%3F", "%40", "%5C", "%7C", "%7D", "%7B")
    val arr1 = List(" ", "!", "\"", "#", "$", "&", "'", "(", ")", "*", "+", ",",  ":", ";", "<", "=", ">", "?", "@", "\\", "|", "}", "{")
  ...
  def getFileName(): Array[String] = {
    ...
    fileName.foreach { eachfile =>
      ...
      prefix = eachFileName.head.replace(filePath, "")
