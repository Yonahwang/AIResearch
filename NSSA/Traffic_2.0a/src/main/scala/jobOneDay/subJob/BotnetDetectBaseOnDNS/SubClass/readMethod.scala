...
trait readMethod extends loggerSet{
  ...
  def readDataFromHDFS(spark: SparkSession, properties: Properties) = {
    ...
    val path = properties.getProperty("hdfs.path") + properties.getProperty("hdfs.clean.data.path") + "dns/"
    val hdfs = FileSystem.get(URI.create(path), conf)
    val fileList: Array[FileStatus] = hdfs.listStatus(new Path(path))
    //获取hdfs目录文件(过滤最后一个文件)
...
//  //读取HDFS文件
//  def readDataFromHDFS(spark: SparkSession, properties: Properties) = {
//    val options = Map("header" -> "true", "delimiter" -> "\t", "path" -> properties.getProperty("hdfs.netflow.path"))
