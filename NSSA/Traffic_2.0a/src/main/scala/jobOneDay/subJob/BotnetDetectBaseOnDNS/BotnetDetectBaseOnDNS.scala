...
object BotnetDetectBaseOnDNS extends Serializable with loggerSet with saveMethod with readMethod with saveToKAFKA{
  ...
  def main(args: Array[String]): Unit = {
    ...
    var ipstream = Try(new BufferedInputStream(new FileInputStream(new File("src/main") + "/conf/botnetBaseOnDNS.properties")))
      .getOrElse(new BufferedInputStream(new FileInputStream(new File(".") + "/conf/botnetBaseOnDNS.properties")))
    ...
    val labelDomainMap: Map[String, Map[String, (Double, Double)]] = clusterResult._1
    ...
    if(resultCount > 1){
      ...
      //保存到kafka
//      toKafka(spark, (properties.getProperty("kafka.nodes"),properties.getProperty("kafka.topic.assetsfind")), botnetResult)
      //保存到postgre
